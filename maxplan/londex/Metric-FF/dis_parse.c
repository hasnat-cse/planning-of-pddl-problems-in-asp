...

{
  ...
  for ( tyl = dis_gparse_types; tyl; tyl = tyl->next ) {
    ...
    if ( tyl->type->next ) {
      tmp = dis_new_dis_Token( dis_MAX_LENGTH );
  ...
  for ( tyl = dis_gparse_constants; tyl; tyl = tyl->next ) {
    ...
    if ( tyl->type->next ) {
      fl->item->next->item = dis_new_dis_Token( dis_MAX_LENGTH );
  ...
  for ( tyll = dis_gparse_predicates; tyll; tyll = tyll->next ) {
    ...
    for ( tyl = tyll->args->next; tyl; tyl = tyl->next ) {
      ...
      if ( tyl->type->next ) {
	tmp_tl->item = dis_new_dis_Token( dis_MAX_LENGTH );
...

{
  ...
  case dis_EX:
    ...
    if ( (*n)->parse_vars->type->next ) {
      tmp_tl->next->item = dis_new_dis_Token( dis_MAX_LENGTH );
