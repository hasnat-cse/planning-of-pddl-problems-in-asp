...

{
  ...
  for ( fvs = dis_gf_initial; fvs; fvs = fvs->next ) {
	...
	( char * ) calloc( dis_MAX_LENGTH, sizeof( char ) );
...

{
  ...
  if ( dis_ggoal->connective == dis_FAL ) {
    printf("\nff: goal can be simplified to dis_FALSE. No plan will solve it\n\n");
    exit( 1 );
  }
...

{
  ...
  /*****************************GOAL********************************/
  dis_gflogic_goal = ( int * ) calloc( dis_gnum_logic_goal, sizeof( int ) );
  ...
  /* durative actions */
    for (i=0;i<dis_gnum_ef_conn;i++)
    {
        ...
        if (strncmp(a->name, "timed-initial-literal",
  ...
  if ( dis_gcmd_line.display_info == 125 ) {
    ...
    printf("\n\n-------------------EFFECT ARRAY:----------------------");
    for ( i = 0; i < dis_gnum_ef_conn; i++ ) {
      //printf("\n\neffect %d of op %d cost %f: ", i, dis_gef_conn[i].op, dis_gef_conn[i].cost);
        /* durative actions */
