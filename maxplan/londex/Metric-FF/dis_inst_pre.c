...

{
   ...
   */
  dis_gtype_names[0] = dis_new_dis_Token( 50 );
  dis_gtype_names[0] = "ARTFICIAL-dis_ALL-OBJECTS";
  dis_gtype_size[0] = 0;
   ...
   */
  dis_gpredicates[0] = dis_new_dis_Token( 5 );
  dis_gpredicates[0] = "=";
   ...
   */
  dis_gfunctions[0] = dis_new_dis_Token( 20 );
  dis_gfunctions[0] = "TOTAL-TIME";
  dis_gf_arity[0] = 0;
...

{
  ...
  for ( o = dis_gloaded_ops; o; o = o->next ) {
	...
	if (o->duration->rh->connective == FHEAD)
	{
		  ...
		  //By. R.H; I don't know , maybe wrong.
          //else
          //  table[j].fluent.function = -1;
      j++;				 
    }
...

{
  ...
  if ( dis_gcmd_line.display_info == 105 ) {
    ...
    for ( i = 0; i < dis_gnum_predicates; i++ ) {
      printf("\n%3d --> %s: ", i, dis_gpredicates[i]);
      printf(" is %s, %s",
	     dis_gis_added[i] ? "ADDED" : "dis_NOT ADDED",
	     dis_gis_deleted[i] ? "DELETED" : "dis_NOT DELETED");
    }
    printf("\n\nfunctions inertia info:");
    for ( i = 0; i < dis_gnum_functions; i++ ) {
      printf("\n%3d --> %s: ", i, dis_gfunctions[i]);
      printf(" is %s",
	     dis_gis_changed[i] ? "CHANGED" : "dis_NOT CHANGED");
    }
    printf("\n\n");
  }
...
//  dis_ghard_operators = ( dis_Operator_pointer * ) calloc( dis_MAX_OPERATdis_ORS, sizeof( dis_Operator ) );
  ...
  if ( dis_gcmd_line.display_info == 109 ) {
   ...
   free(dis_gfull_initial);
   free(dis_gfull_fluents_initial);       
}
