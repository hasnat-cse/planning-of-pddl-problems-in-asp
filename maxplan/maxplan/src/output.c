...
/*  used by cg_handling by X.Z.*/
void get_ft_name( int index, char *str )

{

  get_Fact( &(grelevant_facts[index]),str );

}


/* Used by cg_handleing by X.Z.*/
void get_op_name( int index, char * str)
{
  int i;
  ...
  if ( gop_conn[index].noop_for == -1 ) {
    ...
    if ( goperators[gop_conn[index].op]->name ) {
      strcpy(str, goperators[gop_conn[index].op]->name);
    }
    
    for ( i=0; i<goperators[gop_conn[index].op]->num_vars; i++ ) {
    	strcat(str,"-");
		strcat(str, gconstants[gop_conn[index].inst_table[i]]);
    }
...

{
  ...
  strcpy(str, gpredicates[f->predicate]);
   
  for ( j=0; j<garity[f->predicate]; j++ ) {
    if ( f->args[j] >= 0 ) {
    	strcat(str, "-");
      strcat(str, gconstants[(f->args)[j]]);
      
    } else {
    	strcat(str, "-");
    	sprintf(temp,"%s",DECODE_VAR( f->args[j]));
      strcat(str,temp);
