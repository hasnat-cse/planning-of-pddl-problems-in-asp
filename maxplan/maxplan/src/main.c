...
{
	printf("MaxPlanII beta.\n");
	long solver_start_clk = (long)clock();
  ...
  char cg_domain[MAX_LENGTH] = "";
  char cg_problem[MAX_LENGTH] = "";
  sprintf(cg_domain,"%s%s",gcmd_line.path,gcmd_line.cg_domain_file_name);
  sprintf(cg_problem,"%s%s",gcmd_line.path,gcmd_line.cg_problem_file_name);
  
  strncpy( gcmd_line.cg_domain_file_name, cg_domain, MAX_LENGTH);
  strncpy( gcmd_line.cg_problem_file_name, cg_problem, MAX_LENGTH);
  ...
  /* This is for the action-based encoding; 
	if( gcmd_line.printEncoding!=-1 ){
		gcmd_line.cnflayer = gcmd_line.printEncoding;
		init_solver();
		execute_solver(0);
		exit(0);
	}
  */
	...
	int maxLevel = 100,layer;
	int res = UNSAT;

	gcmd_line.makeCNF = 0;
	for( layer = 1 ; layer<maxLevel ;layer++){
		printf( "\n******  Solve with %d layers Start******\n", layer );
		gcmd_line.cnflayer = layer;
		
		init_solver();
		res = execute_solver(0);
		post_solving(res);
		if( res == SAT ) break; 
		//output_planner_info();
	}
	...
	if( res == SAT ){
		...
		printf("Cannot find a valid solution.");
}
...
/*
  ...
  printf( "\n            %7.2f seconds total planner time (solving not included)\n\n", 
	...
	printf("\nInfo:   %7.2lf seconds global time", get_global_time());
	printf("\n        %7.2lf seconds total encoding time", encoding_time);
	printf("\n        %7.2lf seconds total SAT solving time", solving_time);
	printf("\n        %7.2f MB memory used",max_memory_usage);
	printf("\n            solution found at level %d",gcmd_line.cnflayer);
	//printf("\nOther:");
	//printf("\n        %ld londex constraints",londex_count);
	printf("\n\n");
}

void output_planner_info_file(FILE* fp, int numSteps) {
    
    fprintf(fp, "; Time %7.2lf\n", get_global_time());
...
/*
 * fixed solving_result name by R.H.bb
 */
...
int create_solution() {
    ...
    //strcpy(gcmd_line.input_solution,"bb_solving_result_cnf.tmp");
    //if ( (fp = fopen( gcmd_line.input_solution, "r" )) == NULL ) {    
    if ( (fp = fopen( "bb_solving_result_cnf", "r" )) == NULL ) {
